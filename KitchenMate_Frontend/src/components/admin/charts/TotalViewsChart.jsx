/**
 * Total Views Chart - KitchenMate Admin
 *
 * LineChart showing views over 7 days
 */
  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] px-3 py-2 shadow-lg">
      ...
      <p className="text-sm text-[var(--color-secondary)]">
        Lượt xem: <span className="font-semibold">{payload[0]?.value ?? 0}</span>
      </p>
    </div>
  )
}

export function TotalViewsChart({ data = [], loading = false, error = null, onRetry }) {
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <h3 className="font-display font-semibold text-[var(--color-text)]">Lượt xem công thức</h3>
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-64 flex items-center justify-center text-center">
            <p className="text-sm text-[var(--color-text-secondary)]">Chưa có dữ liệu lượt xem</p>
          </div>
        </CardContent>
      </Card>
    )
